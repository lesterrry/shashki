...
const C_M_Y_K = ["#00AAE9", "#D92D8A", "#FFF34A", "#000000"];
const CM_M_CY_CMY = ["#2F308C", "#D92D8A", "#00A359", "#363639"];
const C_MY_CY_CMY = ["#00AAE9", "#DA3832", "#00A359", "#363639"];
const CM_MY_Y_CMY = ["#2F308C", "#DA3832", "#FFF34A", "#363639"];

const SCROLL_REGIONS = [
  // todo длины вместо диапазонов
...
const SCROLL_SUBREGIONS = [4, 3, 2, 0, 3, 14, 12, 0, 0, 0, 4, 0, 0, 11, 5, 0];
...
const createRandomColor = (colorDict, index) => {
  return colorDict["makeCheckers"] && index % 2 === 0
    ? "rgba(255,255,255,0)"
    : colorDict["array"].random();
};
...
const resolveColor = (color, arrayIndex = 0, randomIndex = 0) => {
  ...
  return Array.isArray(color)
    ? color[arrayIndex]
    : createRandomColor(color, randomIndex);
};
...
const createFlock = (
  ...
  if (position)
    $flock.css({ top: calc(50, position[0]), left: calc(50, position[1]) });
...
const repaintFlock = (id, color) => {
  ...
  $children.each(function (i) {
    ...
    $(this).css({
      "background-color": resolveColor(color, 0, row + column),
      filter: "",
    });
  });
};
...
const animate = (currentTime) => {
  ...
  if (deltaTime >= 1) {
    if (currentScrollRegion === -1)
      repaintFlock("s", { array: C_M_Y_K, makeCheckers: true });
    if (currentScrollRegion === 3 && subRegion === 0)
      repaintFlock("m", { array: CM_M_CY_CMY, makeCheckers: true });
    if (currentScrollRegion === 8 && subRegion === 0)
      repaintFlock("m2", { array: C_MY_CY_CMY, makeCheckers: true });
    if (currentScrollRegion === 15 && subRegion === 0)
      repaintFlock("s2", { array: CM_MY_Y_CMY, makeCheckers: true });
...
const rollback = (affected) => {
  for (const i of affected) {
    ...
    modifications = modifications.filter(
      (j) =>
        j.region !== i.region ||
        (j.region === i.region && j.subRegion !== i.subRegion),
    );
  }
};
...
const rollbackBySubRegion = (priorSubRegion, priorRegion) => {
  const affected = modifications.filter(
    (i) => i.subRegion === priorSubRegion && i.region === priorRegion,
  );
  rollback(affected);
};
  ...
  switch (region) {
    case 0:
      for (let i = 0; i < slice_s.length; i++) {
        ...
        $(slice_s[i]).css(
          "filter",
          `grayscale(${slice_s.length * multiplier * 100}%)`,
        );
      }
      ...
      if (!subRegionApplied) {
        switch (subRegion) {
          case 0:
            modify(
              $cell_a,
              "width: 400px; height: 160px; z-index: 40; background-color: #EEEEEE",
            );
          ...
          case 1:
            ...
            modify(
              $(getCell("s", 7, 9)),
              `background-image: url(${TAXI_1}); filter: initial`,
            );
            modify(
              $(getCell("s", 4, 7)),
              `background-image: url(${TAXI_2}); filter: initial`,
            );
            modify(
              $(getCell("s", 3, 4)),
              `background-image: url(${TAXI_3}); filter: initial`,
            );
            modify(
              $(getCell("s", 0, 1)),
              `background-image: url(${TAXI_4}); filter: initial`,
            );
            modify(
              $(getCell("s", 7, 2)),
              `background-image: url(${TAXI_5}); filter: initial`,
            );
            modify(
              $(getCell("s", 1, 12)),
              `background-image: url(${TAXI_6}); filter: initial`,
            );
            break;
          case 2:
            ...
            modify(
              $(getCell("s", 7, 9)),
              `background-image: none; background-color: #FFF34A`,
            );
            modify(
              $(getCell("s", 4, 7)),
              `background-image: none; background-color: #FFF34A`,
          ...
          case 3:
            ...
            modify(
              $(getCell("s", 5, 6)),
              `filter: initial; background-color: #FFF34A`,
            );
            modify(
              $(getCell("s", 6, 10)),
              `filter: initial; background-color: #FFF34A`,
    ...
    case 2:
      for (let i = 0; i < slice_s.length; i++) {
        let $filteredElements = $(slice_s[i]).filter(function () {
          return !$(this).is(".cell#b");
        });
      ...
      if (!subRegionApplied) {
        switch (subRegion) {
          case 0:
            modify(
              $(getCell("xs", 0, 1)),
              `background-image: url(${TAXI_7}); filter: initial`,
            );
            break;
          case 1:
            modify(
              $(getCell("xs", 1, 2)),
              `background-image: url(${TAXI_8}); filter: initial`,
            );
            break;
          case 2:
            modify(
              $(getCell("xs", 0, 3)),
              `background-image: url(${TAXI_9}); filter: initial`,
            );
            break;
          case 3:
            modify(
              $(getCell("xs", 1, 4)),
              `background-image: url(${TAXI_10}); filter: initial`,
            );
            break;
          case 4:
            modify(
              $(getCell("xs", 0, 5)),
              `background-image: url(${TAXI_11}); filter: initial`,
            );
            break;
          case 5:
            modify(
              $(getCell("xs", 1, 6)),
              `background-image: url(${TAXI_12}); filter: initial`,
            );
            break;
          case 6:
            modify(
              $(getCell("xs", 0, 7)),
              `background-image: url(${TAXI_13}); filter: initial`,
            ); // 13
            break;
          case 7:
            modify(
              $(getCell("xs", 1, 8)),
              `background-image: url(${TAXI_14}); filter: initial`,
            );
            break;
          case 8:
            modify(
              $(getCell("xs", 0, 9)),
              `background-image: url(${TAXI_15}); filter: initial`,
            );
            break;
          case 9:
            modify(
              $(getCell("xs", 1, 10)),
              `background-image: url(${TAXI_16}); filter: initial`,
            );
            break;
          case 10:
            modify(
              $(getCell("xs", 0, 11)),
              `background-image: url(${TAXI_17}); filter: initial`,
            ); // memento
            break;
          case 11:
      ...
      if (!subRegionApplied) {
        switch (subRegion) {
          ...
          case 3:
            ...
            modify(
              $(getCell("xs2", 3, 8)),
              "z-index: 30; scale: 2.5; border-radius: 10px; filter: initial",
            );
            modify(
              $(getCell("xs2", 5, 4)),
              "z-index: 30; scale: 2.5; border-radius: 10px; filter: initial",
          ...
          case 4:
            ...
            modify(
              $(getCell("xs2", 3, 8)),
              "scale: initial; border-radius: initial",
            );
            modify(
              $(getCell("xs2", 5, 4)),
              "scale: initial; border-radius: initial",
            ...
            modify(
              $(getCell("xs2", 3, 10)),
              "z-index: 31; scale: 2.5; border-radius: 10px; filter: initial",
            );
            modify(
              $(getCell("xs", 0, 7)),
              "z-index: 31; scale: 2.5; border-radius: 10px; filter: initial",
          ...
          case 5:
            ...
            modify(
              $(getCell("xs", 0, 11)),
              "z-index: 32; scale: 2.5; border-radius: 10px; filter: initial",
            );
            break;
          case 6:
            ...
            modify(
              $(getCell("xs2", 4, 5)),
              "z-index: 33; scale: 2.5; border-radius: 10px; filter: initial",
            );
            break;
          case 7:
            ...
            modify(
              $(getCell("xs", 1, 2)),
              "z-index: 34; scale: 2.5; border-radius: 10px; filter: initial",
            );
            break;
          case 8:
            ...
            modify(
              $(getCell("xs2", 4, 9)),
              "z-index: 35; scale: 2.5; border-radius: 10px; filter: initial",
            );
            break;
          case 9:
            ...
            modify(
              $(getCell("xs", 0, 9)),
              "z-index: 36; scale: 2.5; border-radius: 10px; filter: initial",
            );
            break;
          case 10:
    ...
    case 8:
      ...
      modify(
        $frame,
        `width: 1320px; height: 660px; opacity: 100%; z-index: 10`,
      );
      ...
      if (!subRegionApplied) {
        switch (subRegion) {
          case 1:
            ...
            modify(
              "div#int-1",
              "transform: scale(1); rotate: 4deg; opacity: 100%",
            );
            break;
          case 2:
          ...
          case 3:
            ...
            modify(
              "div#int-2",
              "transform: scale(1); rotate: -2deg; opacity: 100%",
            );
            break;
          case 4:
          ...
          case 7:
            ...
            modify(
              "div#int-4",
              "transform: scale(1); rotate: 6deg; opacity: 100%",
            );
            break;
          case 8:
...
$(document).on("scroll", function () {
  ...
  const getRegion = (value) => {
    ...
    for (let i = 0; i < SCROLL_REGIONS.length; i++) {
      if (value >= SCROLL_REGIONS[i][0] && value <= SCROLL_REGIONS[i][1])
        return i;
    }
    return 1000;
  };

  let currentScrollRegionProgress = () => {
    if (currentScrollRegion === -1) return 0;
    if (currentScrollRegion === 1000) return 1;
    return mapValue(
      currentScrollPx,
      SCROLL_REGIONS[currentScrollRegion][0],
      SCROLL_REGIONS[currentScrollRegion][1],
      0,
      1,
    );
  };
  ...
  const newSubRegion = Math.round(
    mapValue(
      currentScrollRegionProgress(),
      0,
      1,
      0,
      SCROLL_SUBREGIONS[currentScrollRegion],
    ),
  );
...
window.addEventListener("load", () => {
  ...
  createFlock(
    ...
    8,
    16,
    { array: C_M_Y_K, makeCheckers: true },
    false,
    true,
  );
  slices["s"] = sliceFlock("s");

  createParagraphInCell(
    "a",
    "s",
    1,
    3,
    "Миллионы заказов ежедневно, десятки тысяч машин и&nbsp;невырезаемый <nobr>желто-черный</nobr> фон, навсегда сросшийся с&nbsp;москвой&nbsp;&mdash; такси везде и&nbsp;повсюду, в&nbsp;каждом дворе и&nbsp;на&nbsp;каждом перекрестке.",
  );
  createParagraphInCell(
    "a",
    "s",
    5,
    9,
    "Эта тихая экспансия приучила людей не&nbsp;замечать ни&nbsp;водителей такси, ни&nbsp;их&nbsp;автомобили. Тем временем для самих таксистов машина&nbsp;&mdash; и&nbsp;верный друг, и&nbsp;бессменный попутчик, и&nbsp;надежный кормилец, и&nbsp;&mdash; временами&nbsp;&mdash; уютный дом.",
  );
  ...
  createParagraphInCell(
    ...
    1,
    3,
    "Разумеется, как любой дом, автомобиль постепенно обрастает чертами своего хозяина, и&nbsp;начинает походить на&nbsp;него",
  );
  createParagraphInCell(
    "b",
    "s",
    5,
    9,
    "Я&nbsp;решил исследовать эту тему и&nbsp;месяц ездил по&nbsp;москве в&nbsp;надежде отыскать среди таксистов отголосок коллективного разума&nbsp;&mdash; нечто всевоплощающее и&nbsp;единое, душу безустанных московских извозчиков.",
  );
  ...
  createFlock(
    ...
    11,
    22,
    { array: CM_M_CY_CMY, makeCheckers: true },
    true,
    false,
  );
  ...
  createFlock(
    ...
    6,
    13,
    { array: ["#EEEEEE"], makeCheckers: true },
    true,
    false,
    [-540, -640],
  );
  ...
  $(getCell("xs2", 4, 5)).css({
    "background-image": `url(${TAXI_18})`,
    filter: "grayscale(100%)",
  }); // бэтмен
  $(getCell("xs2", 4, 7)).css({
    "background-image": `url(${TAXI_19})`,
    filter: "grayscale(100%)",
  }); // перепрыгни
  $(getCell("xs2", 5, 4)).css({
    "background-image": `url(${TAXI_20})`,
    filter: "grayscale(100%)",
  }); // дагестан
  $(getCell("xs2", 3, 8)).css({
    "background-image": `url(${TAXI_21})`,
    filter: "grayscale(100%)",
  }); // россия
  $(getCell("xs2", 5, 10)).css({
    "background-image": `url(${TAXI_22})`,
    filter: "grayscale(100%)",
  }); // озеро
  $(getCell("xs2", 4, 9)).css({
    "background-image": `url(${TAXI_23})`,
    filter: "grayscale(100%)",
  }); // здарова
  $(getCell("xs2", 3, 10)).css({
    "background-image": `url(${TAXI_24})`,
    filter: "grayscale(100%)",
  }); // гонщик
  $(getCell("xs2", 3, 6)).css({
    "background-image": `url(${TAXI_25})`,
    filter: "grayscale(100%)",
  }); // голд
  $(getCell("xs2", 5, 2)).css({
    "background-image": `url(${TAXI_26})`,
    filter: "grayscale(0%)",
  }); // конь
  ...
  createFlock(
    ...
    11,
    22,
    { array: C_MY_CY_CMY, makeCheckers: true },
    true,
    false,
  );
  ...
  createFlock(
    ...
    6,
    12,
    { array: CM_MY_Y_CMY, makeCheckers: true },
    true,
    false,
  );
  ...
  if (
    ...
    let image = $("<img>").attr("id", "buttplug").attr("src", HAHA);
    $body.append(image);
  } else {
    let timeout = 2000;
    if (
      /(?!.*\b(Edge|Edg|OPR|FxiOS|CriOS)\b) AppleWebKit.*Chrome/.test(
        navigator.userAgent,
      )
    ) {
      timeout = 0;
